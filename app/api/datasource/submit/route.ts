...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!payload || !cookie) {
            return NextResponse.json({ success: false, message: 'Missing payload or cookie' }, { status: 400 });
        }
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_DATASOURCE_URL}/form_bapp/submit`, {
            method: 'POST',
            headers: {
        ...
        return NextResponse.json({ success: true, message: 'Submitted', debug: text.substring(0, 100) });
